...
class DuelingGNN(nn.Module):
    ...
    def __init__(self, input_dim: int, hidden_dim: int, output_dim: int, num_hops: int = 2):
    ...
    def get_subgraph(self, x: torch.Tensor, edge_index: torch.Tensor, current_node: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, current_node: int) -> torch.Tensor:
        # Get k-hop subgraph
...
class DuelingGraphDQN(nn.Module):
    ...
    def forward(self, state: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        cls_logits = cls_value + (cls_advantage - cls_advantage.mean(dim=-1, keepdim=True))
